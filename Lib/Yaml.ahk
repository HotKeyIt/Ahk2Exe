...
{
If !_CNT&&(A_LoopField=""||RegExMatch(A_LoopField,"^\s+$")){
...
_LVL:=Yaml_S2I(_LVL)
If _LVL-__LVL>(__SEQ?2:1)||(_LVL>__LVL&&_LVLChanged)
_LVL:=__LVL+1+(__SEQ?1:0),_LVLChanged:=_LVL
else if _LVLChanged
_LVL:=_LVLChanged
else _LVLChanged:=0
If maxLVL<_LVL
...
If J!=""&&J<=R{
dump.=(F?(%F%1 Yaml_Dump(value,J,R+1,F) %F%2):Yaml_EscIfNeed(value)) (I=MX&&(!M||seq=O)?E:C)
} else if ((dump:=dump N Yaml_I2S(R) S)||1)&&F
dump.= (J!=""&&J<=(R+1)?%F%1:E) Yaml_Dump(value,J,R+1,F) (J!=""&&J<=(R+1)?%F%2:E)
else dump .= Yaml_EscIfNeed(value)
}
}
I:=0
If seq!=O
